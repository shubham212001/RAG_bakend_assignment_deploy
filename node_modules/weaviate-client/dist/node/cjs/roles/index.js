...
const roles = (connection) => {
  return {
    ...
    ofCurrentUser: () => connection.get('/authz/users/own-roles').then(util_js_1.Map.roles),
    byName: (roleName) => connection.get(`/authz/roles/${roleName}`).then(util_js_1.Map.roleFromWeaviate),
    byUser: (user) => connection.get(`/authz/users/${user}/roles`).then(util_js_1.Map.roles),
    assignedUsers: (roleName) => connection.get(`/authz/roles/${roleName}/users`).then(util_js_1.Map.users),
    create: (roleName, permissions) => {
      const perms = util_js_1.Map.flattenPermissions(permissions).map(util_js_1.Map.permissionToWeaviate);
      return connection
    ...
    hasPermission: (roleName, permission) =>
      connection.postReturn(
        `/authz/roles/${roleName}/has-permission`,
        util_js_1.Map.permissionToWeaviate(permission)
      ),
  };
};
exports.permissions = {
