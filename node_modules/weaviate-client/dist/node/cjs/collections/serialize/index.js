...
Serialize.common = (args) => {
  ...
  if (args === null || args === void 0 ? void 0 : args.rerank) {
    out.rerank = _a.rerank(args.rerank);
  }
  return out;
};
Serialize.fetchObjects = (args) => {
  return Object.assign(Object.assign({}, _a.common(args)), {
    after: args === null || args === void 0 ? void 0 : args.after,
    sortBy: (args === null || args === void 0 ? void 0 : args.sort) ? _a.sortBy(args.sort.sorts) : undefined,
  });
};
  ...
  if (_a.isHybridVectorSearch(vector)) {
    ...
    return vectorBytes !== undefined
      ...
      : {
          targetVectors,
          targets,
          nearVector: search_get_js_1.NearVector.fromPartial({
            vectorForTargets,
            vectorPerTarget,
          }),
        };
    ...
    return {
      targets,
      targetVectors,
      nearText: search_get_js_1.NearTextSearch.fromPartial({
        query: typeof vector.query === 'string' ? [vector.query] : vector.query,
        certainty: vector.certainty,
        distance: vector.distance,
        moveAway: vector.moveAway
          ? search_get_js_1.NearTextSearch_Move.fromPartial(vector.moveAway)
          : undefined,
        moveTo: vector.moveTo ? search_get_js_1.NearTextSearch_Move.fromPartial(vector.moveTo) : undefined,
      }),
    };
...
Serialize.targetVector = (args) => {
  if (args.targetVector === undefined) {
    ...
    return args.supportsTargets
      ? {
          targets: search_get_js_1.Targets.fromPartial({
      ...
      const vectorForTargets = Object.entries(args.vector)
        ...
        .reduce((acc, { target, vector }) => {
          return utils_js_1.ArrayInputGuards.is2DArray(vector)
            ? acc.concat(vector.map((v) => ({ name: target, vectorBytes: _a.vectorToBytes(v) })))
            : acc.concat([{ name: target, vectorBytes: _a.vectorToBytes(vector) }]);
        }, []);
...
Serialize.targets = (targets, supportsWeightsForTargets) => {
  ...
  switch (targets.combination) {
    case 'sum':
      combination = search_get_js_1.CombinationMethod.COMBINATION_METHOD_TYPE_SUM;
      break;
    case 'average':
      combination = search_get_js_1.CombinationMethod.COMBINATION_METHOD_TYPE_AVERAGE;
      break;
    case 'minimum':
      combination = search_get_js_1.CombinationMethod.COMBINATION_METHOD_TYPE_MIN;
      break;
    case 'relative-score':
      combination = search_get_js_1.CombinationMethod.COMBINATION_METHOD_TYPE_RELATIVE_SCORE;
      break;
    case 'manual-weights':
      combination = search_get_js_1.CombinationMethod.COMBINATION_METHOD_TYPE_MANUAL;
      break;
    default:
...
Serialize.nearVector = (args) => {
  return Object.assign(Object.assign({}, _a.common(args)), {
    nearVector: _a.nearVectorSearch(args),
    autocut: args.autoLimit,
  });
};
    ...
    refProperties: refProperties
      ? refProperties.map((property) => {
          return {
            ...
            properties: _a.queryProperties(property.returnProperties),
            metadata: _a.metadata(property.includeVector, property.returnMetadata),
